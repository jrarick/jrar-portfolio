...
![A collapsed Jenga tower](/jenga.jpg)
...
There may be scenarios where you can't enforce content constraints from within a CMS and you'll need to get creative to ensure content displays correctly. Clipping or truncating text can be appropriate if excessive superfluous text compromises the integrity of the layout.

## Putting it together

Creating resilient layouts is ultimately about anticipating change and designing for adaptability. A balanced Jenga tower can withstand the removal of several pieces and a thoughtfully constructed layout can accommodate changes without collapsing. We need to think about not just what works today, but what continues to work smoothly tomorrow. Your future self (and anyone else who touches your code) is going to thank you for taking the time to build stuff the right way.
